    ...
    return `
        <div class="group relative block h-full">
            <div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-stone-100 flex flex-col h-full hover-lift">
                <!-- Image Container -->
                <div class="relative aspect-[3/4] bg-stone-100 cursor-pointer overflow-hidden p-1" onclick="window.location.href='detail.html?id=${product.id}'">
                    <img 
                        loading="${loadingAttr}" 
                        decoding="async"
                        src="${mainImage}" 
...
// 3. Detail Page - Updated for Gallery & Match QuickView Button Style
async function initDetail() {
    ...
    if (product.available) {
        ...
        // Using Rounded Full and consistent styling with Quick View
        waBtn.className = "w-full flex items-center justify-center gap-2 px-6 py-3 rounded-full text-sm font-bold transition-all shadow-lg hover:shadow-xl shadow-green-500/20 transform hover:-translate-y-0.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:from-green-600 hover:to-emerald-700 hover-lift";
        waBtn.innerHTML = `
...
function openQuickView(event, productId) {
    ...
    // Reset Classes - Use same full rounded style as Details Page
    waBtn.className = "w-full flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg hover:shadow-xl hover:from-green-600 hover:to-emerald-700 transition-all hover-lift";
    ...
    if (product.available) {
        ...
        // Add Icon to be consistent with details page
        waBtn.innerHTML = `
